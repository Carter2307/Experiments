...
enum IconColors {
    Blue = "#1C73FF", Green = "#24EC56", Pink = "#E45DFF", Marine = "#28D6FD", Yellow = "#FFE11C", Gray = "#EEEEEE",
}
...
const PromptBox = () => {
    ...
    return (<div
        className=" relative flex flex-row items-center p-[2px] space-x-[5px] h-10 w-60 overflow-hidden rounded-full border border-solid border-gray-200 bg-white">
        <PromptButtonIcon
...
const PromptBoxInput = ({animate}: { animate: boolean }) => {
    ...
    return (<motion.div
        ...
        animate={{
            "--blur": animate ? `10px` : "0px", x: animate ? 32 : 0, opacity: animate ? 0 : 1,
        }}
        transition={{
            type: "spring", stiffness: 80, damping: 10, mass: 1,
        }}
...
const PromptBoxAction = ({animate}: { animate: boolean }) => {
    const buttonIcons = [{
        color: IconColors.Yellow, icon: assets.FileIcon.src,
    }, {
        color: IconColors.Green, icon: assets.PictureIcon.src,
    }, {
        color: IconColors.Pink, icon: assets.FacetimeIcon.src,
    }, {
        color: IconColors.Marine, icon: assets.ShareDotIcon.src,
    },];
    return (<motion.div className="absolute left-8 top-[3px] space-x-[4px] flex flex-row rounded-full z-0">
        {buttonIcons && buttonIcons.map((btn, index) => {
            ...
            return (<motion.div
                ...
                key={btn.icon}
                initial={{
                    x: -distance, rotate: "-90deg", "--blur": "4px", opacity: 0,
                }}
                animate={{
                    ...
                    transition: {
                        default: {
                            type: "spring", stiffness: 80, damping: 12, duration: 1.8,
                        }, "--blur": {delay: 0.1}, rotate: {
                            type: "spring", stiffness: 80, damping: 12, duration: 2, delay: 0.1,
                        },
                    },
                }}
                style={{filter: "blur(var(--blur))"}}
